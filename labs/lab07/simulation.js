...
export function initializeExperiment({
    ...
    const state = {
        isPowered: false,
        selectedModeId: LAB07_EXPERIMENT_MODES[0].id,
    ...
    function renderTable() {
        tableBody.innerHTML = LAB07_EXPERIMENT_MODES.map(
            (mode) => {
        ...
        const completedCount = LAB07_EXPERIMENT_MODES.filter(
            (mode) => Boolean(state.records[mode.id])
        ).length;

        progressValue.textContent =
            `${completedCount} із ${LAB07_EXPERIMENT_MODES.length}`;

        experimentSection.classList.toggle(
            "is-completed",
            completedCount === LAB07_EXPERIMENT_MODES.length
        );

        if (completedCount === LAB07_EXPERIMENT_MODES.length) {
